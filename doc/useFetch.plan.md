...
useSeo({
  ...
  title: () => (data.value && !error.value ? `${data.value.length} Products` : 'Products'),
})
```
...
export type UseFetchOptions<TData, TVariables extends object> = UseFetchOptionsBase<TData, TVariables> &
...
```ts
type VariablesOption<TVariables extends object> = {} extends TVariables
  ? { variables?: MaybeRefOrGetter<UseFetchVariableShape<TVariables>> }
  : { variables: MaybeRefOrGetter<UseFetchVariableShape<TVariables>> }
```
...
useSeo({
  title: () => (!error.value && data.value ? `${data.value.length} Products` : 'Products'),
})
</script>
```
