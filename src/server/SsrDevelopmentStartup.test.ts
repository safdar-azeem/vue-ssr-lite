...
describe('development startup with a broken application runtime', () => {
  it('keeps the listener alive and serves the development error page', async () => {
    ...
    expect(first.body.indexOf('<summary>Show details</summary>'))
      .toBeLessThan(first.body.indexOf('Request: /'))
    expect(first.body.indexOf('Request: /'))
      .toBeLessThan(first.body.search(/Error ID: vssl_[a-f0-9]{16}/))
    expect(first.body).not.toContain('<div class="meta">')
